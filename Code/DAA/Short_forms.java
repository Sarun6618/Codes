...
class Short_forms{
    public List<String> makeShortcutWords(String word){
        ...
        solution(word," ",0,0,res);
        Collections.sort(res);
        return res;
        
        //Implement your logic here.
    }
    public static void solution(String s,String op,int count,int pos,ArrayList<String> res){
        if(pos==s.length()){
            if(count==0){
                res.add(op);
            }
        ...
        solution(s,op,count+1,pos+1,res);
        if(count>0){
            solution(s,op+count+s.charAt(pos),0,pos+1,res);
        }
        else{
            solution(s,op+s.charAt(pos),0,pos+1,res);
        }
    }
}